    ...
    .controller('PostCtrl', ['$scope', '$sails', 'lodash', 'config', 'titleService', 'PostsModel', '$filter',
        'ngTableParams', '$location', '$sce', '$window', function PostController($scope, $sails, lodash, config, titleService, PostsModel, $filter, ngTableParams, $location, $sce, $window) {
            ...
            $scope.$watch("dataset", function () {
                //  $scope.tooloptions.refresh();  $scope.GridMAS2.dataSource.read(); $scope.editor.tooloptions.imageBrowser.read(); $scope.tooloptions.imageBrowser.read();
                //  $scope.$apply();
            ...
            $scope.tooloptions = {
                ...
                imageBrowser: {
                    ...
                    transport: {
                        read: {
                            url: function () {
                                return 'api/galleryaticlesKendo/' + $scope.dataset;
                            },
                            type: 'GET'
                        },
                        //destroy: {
...
// imageUrl: function (e) {return "http://sample3.gtz.com:8013/images/" +$scope.dataset+'Articles/'+ e; }
                        imageUrl: function (e) {
                            // return "http://74.114.164.22/images/" + $scope.dataset + 'Articles/' + e;
                            return "http://localhosts:8013/images/" + $scope.dataset + 'Articles/' + e;
